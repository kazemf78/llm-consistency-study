...
if __name__ == "__main__":
    ...
    print("Starting SimpleQA_rebut_v00_new_llm_gen_judge experiment...", "#"*50)
    main()
...
PYTHONUNBUFFERED=1 uv run src/llm_consistency/pipeline/paraphrase.py \
  ...
  --num-rows 1500 \
  --experiment-flag "rebut_v00_new_llm_gen_judge" \
...
PYTHONUNBUFFERED=1 uv run src/llm_consistency/pipeline/answer_generation.py \
  ...
  PYTHONUNBUFFERED=1 uv run src/llm_consistency/experiments/SimpleQA_rebut_v00_new_llm_gen_judge.py   2>&1 | tee "run__logs/whole_pipeline_SimpleQA_rebut_v00_new_llm_gen_judge_$(date +%b%d_%H%M).log"
