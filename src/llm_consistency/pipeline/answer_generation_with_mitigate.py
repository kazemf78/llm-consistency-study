...
def ANSWER_PROMPT_MITIGATE(llm, batch_paras, model_name, api_models, max_local_tokens, max_local_time, max_api_tokens, api_conc, answer_prompt_name):
    # --- Mitigation Prompt ---
    # MITIGATION_PROMPT_TEMPLATE = (
    #     "You are a helpful assistant. Rewrite the following question to improve its clarity "
    #     "and precision, without changing its original meaning or intent. "
    #     "Output ONLY the rewritten question without any introductory text.\n\n"
    #     "Original Question: {q}\n"
    #     "Rewritten Question:"
    # )
    MITIGATION_PROMPT_TEMPLATE = (
    "Rewrite the following question into a standard, neutral, "
    "and encyclopedic format. Remove slang or complex sentence structures."
    "\n\nQuestion: {q}\nCanonical Question:"
    )
    
    # 1. Ask LLM to rewrite the questions first
    rewrite_prompts = [MITIGATION_PROMPT_TEMPLATE.format(q=q) for q in batch_paras]
    if isinstance(llm, OpenAIAPILLM):
        mitigated_qs = llm.batch(rewrite_prompts, concurrency=api_conc, temperature=0.7) # Slightly higher temp for creative rewriting? Or keep 0 for consistency.
    else:
        mitigated_qs = llm.batch(rewrite_prompts, temperature=0.7)
    
    # Clean up results (basic stripping)
    mitigated_qs = [(m or "").strip() for m in mitigated_qs]
    
    # 2. Prepare final prompts using the REWRITTEN questions
    answer = load_prompt(f"answer/{answer_prompt_name}")
    final_prompts = [answer.format(q=mq) for mq in mitigated_qs]

    return final_prompts, mitigated_qs
...
def run_answer_generation(
    ...
    mitigate: bool = True,  # <--- NEW ARGUMENT
):
    """
    ...
    # Update configuration suffix to distinguish mitigated runs from normal runs
    conf = rp.conf_suffix(temperature=temperature)
    if mitigate:
        conf_mitigated = rp.conf_suffix(temperature=temperature)
        conf_mitigated += "_mitigated"
    ...
    run_config = {
        ...
        "mitigate_enabled": mitigate, # <--- Track config
    }
    ...
    all_models_rows_with_mitigate: List[dict] = []

    if local_models: model_names = api_models + local_models
    else: model_names = api_models
    for model_name in model_names:
        print(f"\n=== Processing {model_name} (Mitigation: {mitigate}) ===")
        ...
        if mitigate:
            final_path_mitigated = rp.answers_file(subset=subset_tag, model=model_name, conf_suffix=conf_mitigated)
            partial_path_mitigated = rp.answers_partial_file(subset=subset_tag, model=model_name, conf_suffix=conf_mitigated)
        ...
        if not force and resume and os.path.exists(final_path):
            ...
            if mitigate:
                existing_mitigated = pd.read_csv(final_path_mitigated)
                all_models_rows_with_mitigate.extend(existing.to_dict("records"))
            continue

        start_idx = 0
        model_rows: List[dict] = []
        model_rows_with_mitigate: List[dict] = []
        ...
        if resume and os.path.exists(partial_path):
            ...
            if mitigate and os.path.exists(partial_path_mitigated):
                existing_df_mitigate = pd.read_csv(partial_path_mitigated)
                model_rows_with_mitigate = existing_df_mitigate.to_dict("records")
        ...
        if start_idx >= len(paras):
            ...
            if mitigate:
                all_models_rows_mitigate.extend(model_rows_mitigate)
            continue
        ...
        # We process prompts inside the loop now to handle mitigation injection
        chunk_counter = start_idx // batch_size
        print(f"Total questions: {len(paras)} | Starting from index {start_idx}")
        
        # Iterate over chunks of the INPUT data
        input_data_slice = list(zip(paras[start_idx:], idxs[start_idx:], origs[start_idx:]))
        
        for start, batch_data in chunks(input_data_slice, batch_size):
            st = time.time()
            
            # Unpack batch data
            batch_paras = [x[0] for x in batch_data]
            # batch_idxs = [x[1] for x in batch_data] # Unused but available
            # batch_origs = [x[2] for x in batch_data] # Unused but available

            # --- MITIGATION MODULE ---
            if mitigate:
                # Mitigation path: rewrite the queries to improve clarity
                final_prompts_with_mitigate, mitigated_qs = ANSWER_PROMPT_MITIGATE(llm, batch_paras, model_name, api_models, max_local_tokens, max_local_time, max_api_tokens, api_conc, answer_prompt_name)
            
            # Standard path: use original/paraphrased question directly
            final_prompts = [ANSWER_PROMPT.format(q=q) for q in batch_paras]

            # --- ANSWER GENERATION ---
            if isinstance(llm, OpenAIAPILLM):
                answers = llm.batch(final_prompts, concurrency=api_conc, temperature=temperature)
                if mitigate:
                    answers_with_mitigate = llm.batch(final_prompts_with_mitigate, concurrency=api_conc, temperature=temperature)
            else:
                answers = llm.batch(final_prompts, temperature=temperature)
                if mitigate:
                    answers_with_mitigate = llm.batch(final_prompts_with_mitigate, temperature=temperature)
            ...
            if mitigate:
                for j, ans in enumerate(answers_with_mitigate):
                    i = start_idx + start + j
                    row = {
                        "model": model_name,
                        "idx": int(idxs[i]),
                    ...
                    # Add mitigation column if enabled
                    if mitigate:
                        row["mitigated_question"] = mitigated_qs[j]
                    
                    model_rows_with_mitigate.append(row)

            chunk_counter += 1
            if chunk_counter % save_every_n_chunks == 0:
                pd.DataFrame(model_rows).to_csv(partial_path, index=False)
                if mitigate:
                    pd.DataFrame(model_rows_with_mitigate).to_csv(partial_path_mitigated, index=False)
                print(f"💾 Saved progress after {chunk_counter} chunks -> {partial_path}")
                print(time.ctime())
            print(f"Batch time (Mitigate={mitigate}):", round(time.time() - st, 2), "sec")
            print(time.ctime())

        # save final
        pd.DataFrame(model_rows).to_csv(final_path, index=False)
        if mitigate:
            pd.DataFrame(model_rows_with_mitigate).to_csv(final_path_mitigated, index=False)
        ...
        all_models_rows.extend(model_rows)
        if mitigate:
            all_models_rows_with_mitigate.extend(model_rows_with_mitigate)
    ...
    if mitigate:
        all_path_mitigate = rp.answers_all_models_file(subset=subset_tag, conf_suffix=conf_mitigated)

    pd.DataFrame(all_models_rows).to_csv(all_path, index=False)
    if mitigate:
        pd.DataFrame(all_models_rows_with_mitigate).to_csv(all_path_mitigate, index=False)
...
def parse_args():
    ...
    parser.add_argument("--question-subset", type=str, default="both", choices=["original_only", "paraphrased_only", "both"])
    ...
    parser.add_argument(
        "--mitigate",
        action="store_true",
        help="Enable mitigation module: Rewrites questions for clarity before answering.",
    )
...
if __name__ == "__main__":
    ...
    run_answer_generation(
        ...
        mitigate=args.mitigate,  # Pass the new argument
    )